...
public class HandlerCommand {
    ...
    public void execute(Bacteria bacteria, int commandCode, WorldContext worldContext) {
        switch (commandCode) {
            case 0,1,2,3,4,5,6,7 -> moveCommand.execute(bacteria, commandCode, worldContext);
            case 8,9,10,11,12,13,14,15 -> catchCommand.execute(bacteria, commandCode, worldContext);
            case 16,17,18,19,20,21,22,23 -> lookCommand.execute(bacteria, commandCode, worldContext);
            case 24,25,26,27,28,29,30,31 -> changeDirectionCommand.execute(bacteria, commandCode, worldContext);
            default -> jumpCommand.execute(bacteria, commandCode, worldContext);
        }
    }
}
